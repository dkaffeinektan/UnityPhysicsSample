    ...
    public partial class MouseHoverSystem : SystemBase
    {
        ...
        protected override void OnUpdate()
        {
                ...
                && EntityManager.HasComponent<RenderMeshArray>(mouseHoverEntity))
            {
                ...
                // its original RenderMeshArray is the one we parked on our singleton last time.
            ...
            if (hasCurrentEntity && EntityManager.HasComponent<MaterialMeshInfo>(mouseHover.CurrentEntity) && EntityManager.HasComponent<RenderMeshArray>(mouseHover.CurrentEntity))
            {
                ...
                // Park the entity's original RenderMeshArray on our own singleton while the entity shows the
                // hover material: it keeps the array's mesh/material storage alive (a RenderMeshArray frees it
                // once no entity references it) and it's what we read back to restore the entity next time.
            ...
            // MouseHover is a value type, so everything above ran on a copy. Write it back to persist the
            // tracking fields we changed. We can't mutate it in place through a ref because the structural
            // changes above (adding/swapping shared components) can move the entity to another chunk.
